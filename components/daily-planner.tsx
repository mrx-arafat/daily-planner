...
export default function DailyPlanner({ initialDate }: DailyPlannerProps) {
  ...
  const [date, setDate] = useState(() => {
    ...
    // Otherwise get today's date in local timezone
  ...
  return (
    <div ref={plannerRef} className="max-w-4xl mx-auto p-3 sm:p-6 bg-white font-serif shadow-md rounded-lg">
      {/* Printable content - excludes smart features */}
      <div ref={printableContentRef} className="bg-white">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 sm:mb-8 gap-4 sm:gap-0">
          <h1 className="text-3xl sm:text-4xl font-light text-gray-600">DAILY PERSONAL PLANNER</h1>
          {!isExporting && (
            <div className="flex gap-2 w-full sm:w-auto">
              <Button onClick={exportToImage} className="flex items-center gap-2 flex-1 sm:flex-auto justify-center">
        ...
        <div className="flex flex-col lg:flex-row gap-6 lg:gap-8">
          ...
          <div className="flex-1">
            <div className="mb-4 flex flex-col sm:flex-row items-start sm:items-center flex-wrap gap-2">
              <h2 className="text-gray-600 uppercase tracking-wide mb-2 sm:mb-0">TODAY&apos;S SCHEDULE</h2>
              {!isExporting && (
              ...
              {timeSlots
                ...
                .map((slot, index) => (
                  <div key={index} className="mb-4 flex items-center">
                    <div className="w-12 sm:w-16 text-gray-600 text-sm sm:text-base">{slot.display}</div>
                    <input
                      type="text"
